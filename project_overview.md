...
| **Giao Dịch Gửi** | `frmGiaoDichGui.html` | Thiếu danh sách nhân viên là ADMIN trong phần gán chỉ tiêu/KPI khi nhập giao dịch gửi. | Điều kiện lọc chỉ tải các nhân viên có `nv.Role === "USER"`, do đó vô tình lọc bỏ các ADMIN (như KS02) có tham gia nhận chỉ tiêu thi đua. | Loại bỏ điều kiện `nv.Role === "USER"`, cho phép tải toàn bộ cán bộ ở trạng thái `ACTIVE` vào dropdown gán KPI. |
